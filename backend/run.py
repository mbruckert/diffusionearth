...
def start():
    ...
    print(request.json)
    upload_type = request.json['upload_type']
    ...
    points, colors, intrinsics, height, width = preprocess(depth_map, color_image)

    user = User()

    user.renders[(0,0)] = GridNode(
        N=GridView(points.copy(), colors.copy(), intrinsics.copy(), height, width, prompt, 0, 0, "0,0 N"),
        NE=GridView(points.copy(), colors.copy(), intrinsics.copy(), height, width, prompt, 0, -45, "0,0 NE"),
        NW=GridView(points.copy(), colors.copy(), intrinsics.copy(), height, width, prompt, 0, 45, "0,0 NW"),
    )
    user.renders[(0,1)] = GridNode(
        N=GridView(points.copy(), colors.copy(), intrinsics.copy(), height, width, prompt, 5, 0, "0,1 N"),
    )
...
def move():
    ...
    if action == 'w': # go forward
        ...
        # get the new view
        node = user.renders.get(tuple(user.position + VIEW_OFFSET_MAP[user.orientation]))
        if node is None:
    ...
    def propagate(user, node, view):
        ...
        if node.views.get(VIEW_MAP[(orientation_num - 1) % 8]) is None: # Left of current view
            node.views[VIEW_MAP[(orientation_num - 1) % 8]] = GridView(*view.propagation_data, tf=0, rh=45, grid_id=f"{user.position[0]},{user.position[1]} {VIEW_MAP[(orientation_num - 1) % 8]}")

        if node.views.get(VIEW_MAP[(orientation_num + 1) % 8]) is None: # Right of current view
            node.views[VIEW_MAP[(orientation_num + 1) % 8]] = GridView(*view.propagation_data, tf=0, rh=-45, grid_id=f"{user.position[0]},{user.position[1]} {VIEW_MAP[(orientation_num + 1) % 8]}")

        nn = tuple(user.position + VIEW_OFFSET_MAP[user.orientation]) # Forward from current view
        if user.renders.get(nn) is None:
            ...
            user.renders[nn].views[user.orientation] = GridView(*view.propagation_data, tf=5, rh=0, grid_id=f"{nn[0]},{nn[1]} {user.orientation}")
